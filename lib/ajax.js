...
(function(factory) {
  ...
  extend(MockXhr.prototype, {
    send: function(data) {
      this.params = data;
      this.readyState = 2;
      Ajax._respondForListeners(this);
    },
  ...
  var Ajax = jasmine.Ajax = {
    ...
    useMock: function() {
      if (!Ajax.installed) {
        Ajax.installed = true;
        Ajax._original = $.ajaxSettings.xhr;
    ...
    uninstallMock: function() {
      if (Ajax.installed) {
        $.ajaxSettings.xhr = Ajax._original;
    ...
    listen: function(matcher, response, options) {
      this.listeners.unshift({ matcher: matcher, args: [].slice.call(arguments, 1) });
    },

    _respondForListeners: function(xhr) {
    ...
    _matchUrl: function(matcher, url) {
      if (typeof matcher === 'string') {
        return matcher === url;
      } else {
